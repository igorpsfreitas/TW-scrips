...
function changeTheme(){
    ...
    $("#btn_save_theme").on("click",()=>{
        ...
        UI.MensagemSucesso(`saved colours for: ${selectedTheme} \n run the script again`,1000)
    ...
    $("#btn_reset_theme").on("click",()=>{
        ...
        UI.MensagemSucesso("run the script again",1000)
...
async function balancingResources(){
        ...
        for(let j=0;j<list_coords.length;j++){//for each village of a cluster
            ...
            for(let k=0;k<list_production.length;k++){//search in the main list 
                if(list_production[k].coord == coord){
                    ...
                    let recursos_recebidos=0
                    if(map_incoming.has(coord)){
                        recursos_recebidos=map_incoming.get(coord).madeira+map_incoming.get(coord).argila+map_incoming.get(coord).ferro
                    }
    ...
    for(let i=0;i<list_production.length;i++){
        ...
        if(map_incoming.has(coord)){
            list_production[i].madeira  += map_incoming.get(coord).madeira
            list_production[i].argila += map_incoming.get(coord).argila
            list_production[i].ferro  += map_incoming.get(coord).ferro

            //in case minting a village might have huge cantity of resources underway
            list_production[i].madeira = Math.min(list_production[i].madeira , list_production[i].capacity)
            list_production[i].argila= Math.min(list_production[i].argila, list_production[i].capacity)
            list_production[i].ferro = Math.min(list_production[i].ferro , list_production[i].capacity)

        }
        avg_wood_total +=list_production[i].madeira/list_production.length
        avg_stone_total+=list_production[i].argila/list_production.length
        avg_iron_total +=list_production[i].ferro/list_production.length

        total_wood_home +=list_production[i].madeira
        total_stone_home+=list_production[i].argila
        total_iron_home +=list_production[i].ferro
    }
    ...
    for(let i=0;i<list_production_cluster.length;i++){
        ...
        for(let j=0;j<list_prod.length;j++){
            ...
            avg_wood+=list_prod[j].madeira/list_prod.length
            avg_stone+=list_prod[j].argila/list_prod.length
            avg_iron+=list_prod[j].ferro/list_prod.length

            total_wood_cluster+=list_prod[j].madeira
            total_stone_cluster+=list_prod[j].argila
            total_iron_cluster+=list_prod[j].ferro
        ...
        for(let j=0;j<list_prod.length;j++){
            ...
            let diff_wood =list_prod[j].madeira - Math.round(avg_wood_res)
            let diff_stone =list_prod[j].argila - Math.round(avg_stone_res)
            let diff_iron =list_prod[j].ferro - Math.round(avg_iron_res)

            // console.log(`aici ba prod:${list_prod[j].ferro}, avg: ${Math.round(avg_iron_res)}`)

            //in case diff>0 check if there are available res at home
            diff_wood=(diff_wood < 0)?diff_wood:(list_prod_home[j].madeira - diff_wood > 0)?diff_wood: (list_prod_home[j].madeira)
            diff_stone=(diff_stone < 0)?diff_stone:(list_prod_home[j].argila - diff_stone > 0)?diff_stone: (list_prod_home[j].argila)
            diff_iron=(diff_iron < 0)?diff_iron:(list_prod_home[j].ferro - diff_iron > 0)?diff_iron: (list_prod_home[j].ferro)

            // console.log(`coord ${coord} merch:${merchants} cap:${capacity}, madeira:${diff_wood}, argila:${diff_stone}, ferro:${diff_iron}`)
            ...
            // console.log(`send---->madeira:${send_wood}, argila:${send_stone}, ferro:${send_iron}`)
            ...
            get_wood =(diff_wood>0) ?get_wood :(list_prod[j].madeira +Math.abs(diff_wood) < capacity)? Math.abs(diff_wood) : capacity-list_prod[j].madeira
            get_stone=(diff_stone>0)?get_stone:(list_prod[j].argila+Math.abs(diff_stone)< capacity)? Math.abs(diff_stone): capacity-list_prod[j].argila
            get_iron =(diff_iron>0) ?get_iron :(list_prod[j].ferro +Math.abs(diff_iron) < capacity)? Math.abs(diff_iron) : capacity-list_prod[j].ferro
            // console.log(`get---->madeira:${get_wood}, argila:${get_stone}, ferro:${get_iron}`)
            // console.log("------------------------------------------------------")
            ...
            obj_send.madeira =(send_wood  > 0)?send_wood :0
            obj_send.argila=(send_stone > 0)?send_stone:0
            obj_send.ferro =(send_iron  > 0)?send_iron :0
            if(obj_send.madeira > 0 || obj_send.argila > 0 || obj_send.ferro > 0)
                list_res_send.push(obj_send)
            
            obj_get.madeira =(get_wood  > 0)?parseInt(get_wood) :0
            obj_get.argila=(get_stone > 0)?parseInt(get_stone):0
            obj_get.ferro =(get_iron  > 0)?parseInt(get_iron) :0
            if(obj_get.madeira > 0 || obj_get.argila > 0 || obj_get.ferro > 0)
        ...
        // console.log("avg madeira: " + avg_wood)
        // console.log("avg argila: "+ avg_stone)
        // console.log("avg ferro: " + avg_iron)
        // console.log(`send---> madeira:${total_wood_send}, argila:${total_stone_send}, ferro:${total_iron_send}`)
        // console.log(`get----> madeira:${total_wood_get}, argila:${total_stone_get}, ferro:${total_iron_get}`)
        ...
        for(let j=0;j<list_res_get.length;j++){
            list_res_get[j].madeira =parseInt(list_res_get[j].madeira *norm_wood)
            list_res_get[j].argila=parseInt(list_res_get[j].argila*norm_stone)
            list_res_get[j].ferro =parseInt(list_res_get[j].ferro *norm_iron)
        }
        ...
        for(let j=0;j<list_res_get.length;j++){
            ...
            for(let k=0;k<list_res_send.length;k++){
                ...
                let send_wood=(list_res_send[k].madeira   > 0) ? Math.min(list_res_get[j].madeira  , list_res_send[k].madeira)  : 0
                let send_stone=(list_res_send[k].argila > 0) ? Math.min(list_res_get[j].argila , list_res_send[k].argila)  : 0
                let send_iron=(list_res_send[k].ferro   > 0) ? Math.min(list_res_get[j].ferro  , list_res_send[k].ferro)  : 0



                obj_launch.madeira +=send_wood
                obj_launch.argila +=send_stone
                obj_launch.ferro +=send_iron

                list_res_get[j].madeira -= send_wood 
                list_res_get[j].argila -= send_stone 
                list_res_get[j].ferro -= send_iron

                list_res_send[k].madeira -= send_wood 
                list_res_send[k].argila -= send_stone 
                list_res_send[k].ferro -= send_iron
                ...
                let total_get=list_res_get[j].madeira+list_res_get[j].argila+list_res_get[j].ferro
                if(total_get < minim_resources){
                    // console.log("concluído sending here")
                    break;
                }
            }
        }
    ...
    for(let i=0;i<list_launches.length;i++){
        let nr_merchants=list_launches[i].madeira+list_launches[i].argila+list_launches[i].ferro
    ...
    for(let i=0;i<list_production.length;i++){
        for(let j=0;j<list_launches.length;j++){
            if(list_production[i].coord == list_launches[j].coord_destination){
                list_production[i].madeira +=list_launches[j].madeira
                list_production[i].argila+=list_launches[j].argila
                list_production[i].ferro +=list_launches[j].ferro
            }
            ...
            list_production[i].result_wood =list_production[i].madeira -Math.round(avg_wood_total)
            list_production[i].result_stone=list_production[i].argila-Math.round(avg_stone_total)
            list_production[i].result_iron =list_production[i].ferro -Math.round(avg_iron_total)
    ...
    for(let i=0;i<list_launches.length;i++){
        ...
        let woodKey=`resource[${origin_id}][madeira]`
        let stoneKey=`resource[${origin_id}][argila]`
        let ironKey=`resource[${origin_id}][ferro]`
        let send_resources={}

        //create a map with every launch
        if(map_launches_mass.has(target_id)){
            let obj_update=map_launches_mass.get(target_id)
            obj_update.send_resources[woodKey]=list_launches[i].madeira
            obj_update.send_resources[stoneKey]=list_launches[i].argila
            obj_update.send_resources[ironKey]=list_launches[i].ferro

            obj_update.total_send+=list_launches[i].total_send
            obj_update.total_wood+=list_launches[i].madeira
            obj_update.total_stone+=list_launches[i].argila
            obj_update.total_iron+=list_launches[i].ferro

            obj_update.distance=Math.max(obj_update.distance,list_launches[i].distance)
            map_launches_mass.set(target_id,obj_update)

        }else{
            send_resources[woodKey]=list_launches[i].madeira
            send_resources[stoneKey]=list_launches[i].argila
            send_resources[ironKey]=list_launches[i].ferro
            ...
            map_launches_mass.set(target_id,{
                ...
                total_wood:list_launches[i].madeira,
                total_stone:list_launches[i].argila,
                total_iron:list_launches[i].ferro,
        ...
        if(map_draw_on_map.has(target_id)){
            ...
            obj_update.recursos_recebidos+=list_launches[i].madeira+list_launches[i].argila+list_launches[i].ferro
            map_draw_on_map.set(target_id,obj_update)
        }

        if(map_draw_on_map.has(origin_id)){
            let obj_update=map_draw_on_map.get(origin_id)
            obj_update.recursos_enviados+=list_launches[i].madeira+list_launches[i].argila+list_launches[i].ferro
                    ...
                    success: (data) => {
                        ...
                        if(game_data.device == "desktop"){
                            ...
                            for(let i=0;i<table_production.length;i++){
                                ...
                                let madeira=parseInt(table_production[i].getElementsByClassName("madeira")[0].innerText.replace(".",""))
                                let argila=parseInt(table_production[i].getElementsByClassName("argila")[0].innerText.replace(".",""))
                                let ferro=parseInt(table_production[i].getElementsByClassName("ferro")[0].innerText.replace(".",""))
                            ...
                            for(let i=0;i<table_production.length;i++){
                                ...
                                let madeira = parseInt(table_production[i].getElementsByClassName("mwood")[0].innerText.replace(".",""))
                                let argila = parseInt(table_production[i].getElementsByClassName("mstone")[0].innerText.replace(".",""))
                                let ferro = parseInt(table_production[i].getElementsByClassName("miron")[0].innerText.replace(".",""))
                        ...
                        console.log("espera: "+diff)
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.MensagemSucesso("get production page: "+urls.length)
                        },200-diff)
                    },
                    error: (err)=>{
                        reject(err)
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        for(let i=0;i<table_incoming.length;i++){
                            ...
                            let madeira=parseInt($(table_incoming[i]).find(".madeira").parent().text().replace(".",""))
                            let argila=parseInt($(table_incoming[i]).find(".argila").parent().text().replace(".",""))
                            let ferro=parseInt($(table_incoming[i]).find(".ferro").parent().text().replace(".",""))
                            madeira=(Number.isNaN(madeira) ==true)?0:madeira
                            argila=(Number.isNaN(argila) ==true)?0:argila
                            ferro=(Number.isNaN(ferro) ==true)?0:ferro
                            ...
                            if(map_incoming.has(coord)){
                                ...
                                obj_update.madeira+=madeira
                                obj_update.argila+=argila
                                obj_update.ferro+=ferro
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.MensagemSucesso("get incoming page: "+urls.length)
                        },200-diff)
                    },
                    error:(err)=>{
                        reject(err)
...
async function createTable(list_launches,obj_stats,list_production,list_clusters_stats){
    ...
    let html_prod_table=`
        ...
        <tr>
            ...
            <td class="hide_mobile"><a href="#" id="sort_wood"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/madeira.png"/></a></td>
            <td class="hide_mobile"><a href="#" id="sort_stone"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/argila.png"/></a></td>
            <td class="hide_mobile"><a href="#" id="sort_iron"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/ferro.png"/></a></td>
            <td>send</td>
        </tr>`

    for(let i=0;i<list_launches.length;i++){
        let target_id = list_launches[i].target_id
        let madeira = list_launches[i].total_wood
        let argila = list_launches[i].total_stone
        let ferro = list_launches[i].total_iron
        let origin_id=list_launches[i].id_origin
        ...
        html_prod_table+=`
            <tr id="delete_row" >
                ...
                <td class="hide_mobile">${formatNumber(madeira)}</td>
                <td class="hide_mobile">${formatNumber(argila)}</td>
                <td class="hide_mobile">${formatNumber(ferro)}</td>
    ...
    //hide madeira, argila, ferro for mobile app because there isn't enough space
    if(game_data.device !="desktop")
        $(".hide_mobile").hide()
    ...
    let html_stats_table=`
        ...
        <tr>
            ...
            <td><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/madeira.png"/></td>
            <td><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/argila.png"/></td>
            <td><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/ferro.png"/></td>
        </tr>
        <tr>
    ...
    <tr>
        ...
        <td colspan="2">
            <a href="#" class="order_deficit">
                <center style="margin:10px"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/madeira.png"/></center>
            </a>
        </td>
        <td colspan="2">
            <a href="#" class="order_deficit">
                <center style="margin:10px"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/argila.png"/></center>
            </a>
        </td>
        <td colspan="2">
            <a href="#" class="order_deficit">
                <center style="margin:10px"><img src="https://dsen.innogamescdn.com/asset/c2e59f13/graphic/buildings/ferro.png"/></center>
            </a>
        </td>
        <td >
...
function sendResources(target_id,data) {
    ...
    TribalWars.post("market",options, data, function(response) {
        ...
        UI.MensagemSucesso(response.success,1000)
    }, function(error){
        console.log(error)
    });
}
...
async function getResourcesForAM(time_construction_total,map_farm_usage){
    ...
    let mapa_construções = await getDataBuildings().catch(e=>alert(e))
    ...
    console.log("mapa_construções",mapa_construções)
    ...
    return new Promise((resolve,reject)=>{
        ...
        Array.from(mapa_construções.keys()).forEach(key=>{
            if(key.includes("_time_queued")){
                map_resources_get_AM.set(key.replace("_time_queued",""),{
                    total_wood:0,
                    total_stone:0,
                    total_iron:0,
                    time_finished:Math.round(mapa_construções.get(key)/3600)
                })
            }
        })
        ...
        Array.from(map_coord_templates.keys()).forEach(key=>{//for every coord which have a AM construction template
            let coord=key
            let count_time_construction=mapa_construções.get(coord+"_time_queued")//if a village has already queued building then get time when last building is finished
            ...
            if(mapa_construções.get(coord+"_farm")<30 && map_farm_usage.get(coord)>=0.99){
                let lv_building_HQ=mapa_construções.get(coord+"_main")
                let lv_building_current=mapa_construções.get(coord+"_farm")//curent building from building page
            ...
            for(let i=0;i<list_template.length;i++){//for every building from AM template
                ...
                let lv_building_current=mapa_construções.get(key_building)//curent building from building page
                ...
                if(lv_building_AM>lv_building_current){//means current building must be constructed
                    ...
                    for(let j=0;j<nr_levels;j++){
                        ...
                        mapa_construções.set(key_building,lv_building_current)
            ...
            if (urls.length >= 0 && current_url!="stop") {
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.MensagemSucesso("get AM construction page: "+urls.length)
                        },200-diff)
                    },
                    error:(err)=>{
                        reject(err)
...
function getDataBuildings(){
    ...
    return new Promise((resolve,reject)=>{
        ...
        function ajaxRequest (urls) {
                    ...
                    success: (data) => {
                        ...
                        if(game_data.device == "desktop"){
                            ...
                            for(let i=0;i<table_buildings.length;i++){
                                ...
                                mapa_construções.set(coord+"_time_queued",time_last_construction)
                                ...
                                for(let j=0;j<buildings.length;j++){
                                    ...
                                    mapa_construções.set(key,level)
                                }
                                ...
                                for(let j=0;j<list_queued.length;j++){
                                    ...
                                    if(mapa_construções.has(key)){
                                        let value=mapa_construções.get(key)
                                        mapa_construções.set(key,value+1)
                                    }else{
                                        mapa_construções.set(key,1)
                                    }
                                }
                        ...
                        window.setTimeout(function(){
                            ajaxRequest (list_pages)
                            UI.MensagemSucesso("obtendo página de construção: "+urls.length)
                        },200-diff)
                    },
                    error: (err)=>{
                        reject(err)
            ...
            {
                UI.MensagemSucesso("concluído")
                console.log("mapa_construções herere",mapa_construções)
                resolve(mapa_construções)
...
function getConstantsTwBuildings(){
    if (localStorage.getItem(game_data.world+"constantBuildings") !== null) {
        ...
        console.log("dados de construção já existem")
        return map_constants_buildings
            ...
            for(let i=0;i<list_buildings.length;i++){
                ...
                let madeira=Number(list_buildings[i].getElementsByTagName("madeira")[0].innerText)
                let argila=Number(list_buildings[i].getElementsByTagName("argila")[0].innerText)
                let ferro=Number(list_buildings[i].getElementsByTagName("ferro")[0].innerText)
                ...
                let tempo_construcao=Number(list_buildings[i].getElementsByTagName("tempo_construcao")[0].innerText)
            ...
            console.log("salvar mundo rápido")
        return map_constants_buildings
    }
...
function calculateTimeAndResConstruction(hq, level, obj_data) {
    ...
    var buildTime = obj_data.tempo_construcao * Math.pow(1.2, (level -1)) * Math.pow(1.05, -hq) * constantLvl[level]
    
    let total_wood = Math.round(obj_data.madeira * Math.pow(obj_data.wood_factor, level - 1))
    let total_stone = Math.round(obj_data.argila * Math.pow(obj_data.stone_factor, level - 1))
    let total_iron = Math.round(obj_data.ferro * Math.pow(obj_data.iron_factor, level - 1))
...
function getClusters(data, options) {
    let result_cluster=[]
    let distanciaMaximaGeral=999999;
    let repeat=50;

    for(let rep=0;rep<repeat;rep++){
        let result=insideGetCluster(data,options)
        if(distanciaMaximaGeral > result.maxDistance){
            distanciaMaximaGeral=result.maxDistance
            result_cluster=result
        }
    }
    console.log("distanciaMaximaGeral",distanciaMaximaGeral)
...
function addInfoOnMap(mapInfoResources,random_color){
    ...
    TWMap.mapHandler.spawnSector = function (data, sector) {
        ...
        console.log(`região do mapa carregada`)
                    ...
                    Object.keys(elements).forEach(key=>{
                        ...
                        if(villageId!=null){
                            if(mapInfoResources.has(villageId[0])){
                                ...
                                // console.log(obj)
                                // console.log(`rótulo do agrupamento: ${obj.label_cluster}, color random: `)
    ...
    try {
        ...
        if(document.getElementById(`info_extra${obj.villageId}`) == null ){
            ...
            let html_info=`
                ...
                <center><font color="${greenColor}"  class="shadow20" style="position:absolute;left:${leftImg};top:${topImg};width:14px;height:14px;z-index:4;margin-left:0px;margin-top:11px; font-size: 12px">${parseInt(obj.recursos_recebidos/1000)}k </font></center>
                <center><font color="${redColor}"  class="shadow20" style="position:absolute;left:${leftImg};top:${topImg};width:14px;height:14px;z-index:4;margin-left:0px;margin-top:23px; font-size: 12px">${parseInt(obj.recursos_enviados/1000)}k </font></center>
                `
